...
CREATE TABLE students (
 ...
 ID_of_speciality BIGINT NOT NULL CHECK (ID_of_speciality >= 0) UNIQUE
);
...
CREATE TABLE curses (
 ...
 credits BIGINT NOT NULL CHECK (credits >= 0) UNIQUE,
 description VARCHAR(500) NOT NULL CHECK(LENGTH(description) >= 1)
);

CREATE TABLE history_change_curses (
 ID BIGSERIAL NOT NULL PRIMARY KEY,
 date DATE NOT NULL,
...
CREATE TABLE curses_registration (
...
CREATE TABLE history_change_marks (
 ID_of_curse BIGINT NOT NULL UNIQUE CHECK(ID_of_curse >= 0),
 ID_of_student BIGINT NOT NULL UNIQUE CHECK(ID_of_student >= 0),
 date DATE NOT NULL,
